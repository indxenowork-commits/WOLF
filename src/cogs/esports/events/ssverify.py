...
from core import Cog, Context, WolfRatelimiter
...
class MemberLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = WolfRatelimiter(1, 7)
        return r


class GuildLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = WolfRatelimiter(10, 60)
        return r
...
class Ssverification(Cog):
    def __init__(self, bot: Wolf):
        ...
        self.__mratelimiter = MemberLimits(WolfRatelimiter)  # ss/15s by member
        self.__gratelimiter = GuildLimits(WolfRatelimiter)  # ss/minute by guild
