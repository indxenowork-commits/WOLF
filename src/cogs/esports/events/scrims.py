...
class ScrimEvents(Cog):
    ...
    async def on_scrim_registration(self, message: discord.Message):
        ...
        async with self.__scrim_lock:
            ...
            await scrim.refresh_from_db(("available_slots", "slots_reminder_thresholds", "slots_update_channel_id", "slots_reminder_role_id"))
            remaining_slots = len(scrim.available_slots)
            
            if remaining_slots in scrim.slots_reminder_thresholds and scrim.slots_update_channel:
                embed = discord.Embed(
                    color=0x00FFB3,
                    title=f"⚠️ Only {remaining_slots} Slots Remaining!",
                    description=f"**{scrim.name}** has only **{remaining_slots}** slots left!\n\n"
                                f"Registration Channel: {scrim.registration_channel.mention}\n"
                                f"Hurry up and register before all slots are filled!"
                )
                embed.set_footer(text="Register now to claim your slot!")
                
                content = None
                if scrim.slots_reminder_role:
                    content = scrim.slots_reminder_role.mention
                
                try:
                    await scrim.slots_update_channel.send(content=content, embed=embed)
                except Exception as e:
                    print(f"Failed to send slots reminder: {e}")
