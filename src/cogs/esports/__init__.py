...
from core import Cog, Context, WolfView
from models import *
...
class ScrimManager(Cog, name="Esports"):
    ...
    async def smanager(self, ctx: Context):
        """
        Contains commands related to Wolf's powerful scrims manager.
        """
    ...
    async def tourney(self, ctx: Context):
        """Create & Manage tournaments with Wolf"""
        ...
        view.add_item(WolfView.tricky_invite_button())
    ...
    async def easytag(self, ctx: Context):
        """Commands related to wolf's eztag"""
    ...
    async def set_eztag(self, ctx: Context, *, channel: QuoTextChannel):
        ...
        if count >= 1 and not await ctx.is_premium_user_or_guild():
            return await ctx.error(
                f"Setting up more than 1 EasyTag channel requires either you or this server to have Wolf Premium.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
        ...
        role = discord.utils.get(ctx.guild.roles, name="wolf-tag-ignore")
        if not role:
            role = await ctx.guild.create_role(
                name="wolf-tag-ignore", color=self.bot.color, reason=f"Created by {ctx.author}"
            )
        ...
        embed.description = """
        Unable to mention teammates while registering for scrims or tournaments? Wolf is here for the rescue.

        Use `teammate's ID`, `@teammate_name` or `@teammate's_discord_tag` in your registration format. Wolf will convert that into an actual discord tag.        
        """
    ...
    async def tagcheck_set(self, ctx: Context, channel: discord.TextChannel, mentions: int = 4):
        ...
        if count >= 1 and not await ctx.is_premium_user_or_guild():
            return await ctx.error(
                f"Setting up more than 1 Tagcheck channel requires either you or this server to have Wolf Premium.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
    ...
    async def slotmanager(self, ctx: Context):
        ...
        _view.add_item(WolfView.tricky_invite_button())
    ...
    @commands.command(name="ptable", aliases=("pointstable", "pt"))
    @checks.can_use_tm()
    @checks.is_premium_user_or_guild()
    @commands.bot_has_permissions(embed_links=True, manage_messages=True)
    @commands.cooldown(1, 10, type=commands.BucketType.guild)
    async def points_table(self, ctx: Context):
        """
        Create and manage tournament points tables.
        Track team rankings, kills, and placement points.
        """
        from .views.points import PointsTable
        
        view = PointsTable(ctx)
        view.message = await ctx.send(embed=view.initial_msg, view=view)
    ...
    async def ssverify(self, ctx: Context):
        ...
        """
        if not await ctx.is_premium_user_or_guild():
            if not ctx.guild.member_count > 100 and not ctx.guild.id == 779229001986080779:
                return await ctx.error("Your server must have at least 100 members to setup ssverification, or you need Wolf Premium.")
