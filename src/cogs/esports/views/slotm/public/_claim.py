...
class ClaimSlotModal(discord.ui.Modal, title="Claim Scrims Slot"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        async with claim_lock:
            ...
            # Prompt user to mention teammates in current channel
            prompt_msg = await interaction.followup.send(
                f"**Team Name:** {team_name}\n\n"
                f"Please mention your teammates below in this channel within 60 seconds.\n"
                f"**Format:** @teammate1 @teammate2 @teammate3\n\n"
                f"*If you don't have teammates or don't want to mention them, just wait and the slot will be claimed automatically.*",
                ephemeral=True
            )
            
            # Wait for user's message in current channel
            def check(m):
                return m.author.id == user_id and m.channel.id == interaction.channel_id
            
            _team = {user_id}
            try:
                msg = await scrim.bot.wait_for('message', timeout=60.0, check=check)
                
                # Extract mentioned teammates (excluding bots)
                for mentioned in msg.mentions:
                    if not mentioned.bot:
                        _team.add(mentioned.id)
            ...
            _slot = await AssignedSlot.create(num=num, user_id=user_id, members=list(_team), team_name=team_name)
