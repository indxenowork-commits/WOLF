...
class ScrimsRemind(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> T.Any:
        ...
        if not await self.view.bot.is_premium_user_or_guild(interaction.user.id, interaction.guild_id):
            return await interaction.followup.send(
                "Cancel Reminder feature requires either you or this server to have Wolf Premium.\n\n"
                f"*Purchase [Wolf Premium]({self.view.bot.prime_link}) to use this feature.*",
                ephemeral=True,
            )
        ...
        await interaction.followup.send(
            "Select 1 or multiple scrims to set reminder\n\n*By selecting scrims, you confirm that Wolf can "
