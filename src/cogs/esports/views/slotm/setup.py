...
class ScrimsSlotManagerSetup(EsportsBaseView):
    ...
    async def initial_message(guild: discord.Guild):
        ...
        _e.description = (
            "Slot-Manager is a way to ease-up scrims slot management process. With Wolf's slotm users can - "
    ...
    async def add_channel(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if not await self.ctx.is_premium_user_or_guild():
            if await ScrimsSlotManager.filter(guild_id=self.ctx.guild.id).count() >= 1:
                return await self.ctx.premium_mango("Adding more than 1 Slot-Manager channel requires either you or your server to have Wolf Premium.")
    ...
    async def edit_config(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if _view.custom_id:
            ...
            __editor_view.add_item(WolfView.tricky_invite_button())
