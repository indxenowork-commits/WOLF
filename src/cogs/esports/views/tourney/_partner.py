...
class MediaPartnerView(EsportsBaseView):
    ...
    async def initial_embed(self):
        ...
        embed.description = (
            "With media-partnership you can make Wolf handle media partner registrations "
            "that means in Wolf will check if the user have registered in the partner server "
    ...
    async def add_partner(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if await self.tourney.media_partners.all().count() >= 1 and not await self.ctx.is_premium_user_or_guild():
            return await self.ctx.error(
                "Adding more than 1 media partner requires either you or your server to have Wolf Premium.\n\n"
                f"Buy Wolf Premium here: {self.bot.prime_link}",
                6,
            )
