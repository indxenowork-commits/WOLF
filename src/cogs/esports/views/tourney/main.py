...
class TourneyManager(EsportsBaseView):
    ...
    async def initial_embed(self) -> discord.Embed:
        ...
        _e = discord.Embed(
            color=self.bot.color, title="Wolf Smart Tournament Manager", url=self.bot.config.SERVER_LINK
        )
        ...
        _e.set_footer(
            text="Wolf Prime allows unlimited tournaments.",
    ...
    async def create_tournament(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if not await self.ctx.is_premium_user_or_guild():
            if await Tourney.filter(guild_id=self.ctx.guild.id).count() >= 1:
                return await self.ctx.error(
                    f"Creating more than one tournament requires either you or your server to have Wolf Premium.\n"
                    f"\nBuy Wolf Premium here: {self.bot.prime_link}",
                    7,
                )
    ...
    async def download_excel_data(self, interaction: discord.Interaction, button: discord.Button):
        ...
        #     return await self.ctx.error(
        #         "You need Wolf Premium to download Ms Excel file containing all the "
        #         f"registration data of your tourneys.\n\n"
        #         "Buy Premium for just ₹29 here: https://wolfbot.xyz/premium",
        #         6,
        #     )
