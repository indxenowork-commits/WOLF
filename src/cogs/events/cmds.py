...
class UserCommandLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = cooldown.WolfRatelimiter(2, 10)
        return r
...
class CmdEvents(Cog):
    def __init__(self, bot: Wolf):
        ...
        self.command_ratelimiter = UserCommandLimits(cooldown.WolfRatelimiter)
    ...
    async def bot_check(self, ctx: Context):
        ...
        if self.bot.lockdown is True:
            t = (
                "**Wolf is getting new features** 🥳\n"
                "Dear user, Wolf is updating and is not accepting any commands.\n"
        ...
        with suppress(discord.HTTPException):
            ...
            if not member.bot and record.humans:
                for role in record.humans:
                    try:
                        await member.add_roles(discord.Object(id=role), reason="Wolf's autorole")
