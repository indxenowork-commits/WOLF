...
class MentionLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = cooldown.WolfRatelimiter(2, 12)
        return r


class MainEvents(Cog, name="Main Events"):
    def __init__(self, bot: Wolf) -> None:
        self.bot = bot
        self.mentions_limiter = MentionLimits(cooldown.WolfRatelimiter)
    ...
    async def on_mention(self, ctx: Context) -> None:
        prefix: str = self.bot.cache.guild_data[ctx.guild.id].get("prefix", "w")
        await ctx.send(
