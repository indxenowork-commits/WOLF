...
class Quomisc(Cog, name="quomisc"):
    ...
    async def make_private_channel(self, ctx: Context) -> discord.TextChannel:
        ...
        channel = await guild.create_text_channel(
            "wolf-private", overwrites=overwrites, reason=f"Made by {str(ctx.author)}"
        )
        ...
        e.add_field(
            ...
            inline=False,
            value="This channel is made for Wolf to send important announcements and activities that need your attention. If anything goes wrong with any of my functionality I will notify you here. Important announcements from the developer will be sent directly here too.\n\nYou can test my commands in this channel if you like. Kindly don't delete it , some of my commands won't work without this channel.",
        )
        e.add_field(
            name="**__Important Links__**", value=f"{support_link}", inline=False
        )
    ...
    async def setup_cmd(self, ctx: Context):
        """
        Setup Wolf in the current server.
        This creates a private channel in the server. You can rename that if you like.
        Wolf requires manage channels and manage wehooks permissions for this to work.
    ...
    def format_commit(commit):  # source: R danny
        ...
        return f"[`{short_sha2}`](https://github.com/wolfbot/Wolf-Bot/commit/{commit.hex}) {truncate_string(short,40)} ({offset})"
    ...
    async def about(self, ctx: Context):
        ...
        embed.title = "Wolf Official Support Server"
    ...
    async def footer(self, ctx: Context, *, new_footer: str):
        """Change footer of embeds sent by Wolf"""
    ...
    async def money(self, ctx: Context):
        ...
        e = self.bot.embed(ctx, title="Your Wolf Coins")
        e.set_thumbnail(url=self.bot.user.display_avatar.url)

        e.description = (
            f"💰 | You have a total of `{user.money} Wolf Coins`."
        )
