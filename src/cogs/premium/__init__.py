...
class PremiumCog(Cog, name="Premium"):
    ...
    async def pstatus(self, ctx: Context):
        """Get your Wolf Premium status and the current server's."""
        ...
        embed = self.bot.embed(ctx, title="Wolf Premium", url=f"{self.bot.config.WEBSITE}")
    ...
    async def premium(self, ctx: Context):
        """Checkout Wolf Premium Plans."""
        _e = discord.Embed(
            color=self.bot.color,
            description=f"[**Features of Wolf Pro -**]({self.bot.config.SERVER_LINK})\n\n"
            f"{emote.check} Access to `Wolf Pro` bot.\n"
    ...
    async def on_guild_premium_timer_complete(self, timer: Timer):
        ...
        if (_ch := _g.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
            _e = discord.Embed(
                color=discord.Color.red(), title="⚠️__**Wolf Pro Subscription Ended**__⚠️", url=config.SERVER_LINK
            )
            _e.description = (
                "This is to inform you that your subscription of Wolf Pro has been ended.\n\n"
    ...
    async def on_premium_purchase(self, txnId: str):
        ...
        with suppress(discord.HTTPException, AttributeError):
            _e = discord.Embed(
                color=discord.Color.gold(), description=f"Thanks **{member}** for purchasing Wolf Premium."
            )
        ...
        _e = discord.Embed(
            color=self.bot.color,
            title="Wolf Pro Purchase Successful!",
            url=self.bot.config.SERVER_LINK,
            description=(
                f"{random_greeting()} {member.mention},\n"
                f"Thanks for purchasing Wolf Premium. Your server **{upgraded_guild}** has access to Wolf Pro features until `{_guild.premium_end_time.strftime('%d-%b-%Y %I:%M %p')}`.\n\n"
                "[Click me to Invite Wolf Pro Bot to your server](https://discord.com/oauth2/authorize?client_id=902856923311919104&scope=applications.commands%20bot&permissions=21175985838)\n"
            ),
        )

        if member not in self.bot.server.members:
            _e.description += f"\n\n[To claim your Premium Role, Join Wolf HQ]({self.bot.config.SERVER_LINK})."
