...
class PlanSelector(discord.ui.Select):
    def __init__(self, plans: List[PremiumPlan]):
        super().__init__(placeholder="Select a Wolf Premium Plan... ")
...
class PremiumPurchaseBtn(discord.ui.Button):
    def __init__(self, label="Get Wolf Pro", emoji=emote.diamond, style=discord.ButtonStyle.grey):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send("Please select the Wolf Pro plan, you want to opt:", view=v, ephemeral=True)
        await v.wait()
        ...
        await interaction.followup.send(
            f"You are about to purchase Wolf Premium for **{interaction.guild.name}**.\n"
...
class PremiumView(discord.ui.View):
    def __init__(self, text="This feature requires Wolf Premium.", *, label="Get Wolf Pro"):
    ...
    def premium_embed(self) -> discord.Embed:
        ...
        _e.description = (
            f"\n*`{self.text}`*\n\n"
            "__Perks you get with Wolf Pro:__\n"
