...
class Context(commands.Context["commands.Bot"], Generic[BotT]):
    ...
    async def is_premium_user(self) -> bool:
        ...
        with suppress(AttributeError):
            return (await User.get(user_id=self.author.id)).is_premium

        return False

    async def is_premium_user_or_guild(self) -> bool:
        from models import User, Guild

        with suppress(AttributeError):
            user_check = await User.get_or_none(user_id=self.author.id)
            if user_check and user_check.is_premium:
                return True
            
            guild_check = await Guild.get_or_none(guild_id=self.guild.id)
            if guild_check and guild_check.is_premium:
                return True
    ...
    async def premium_mango(self, msg: str = "This feature requires Wolf Premium.") -> Optional[discord.Message]:
