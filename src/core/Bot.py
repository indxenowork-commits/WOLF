...
on_startup: List[Callable[["Wolf"], Coroutine]] = []


class Wolf(commands.AutoShardedBot):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(
            ...
            activity=discord.Activity(type=discord.ActivityType.listening, name="whelp w setup"),
    ...
    @property
    def prime_link(self):
        return "https://wolfbot.xyz/premium"
    ...
    def reboot(self):
        return os.system("pm2 reload wolf")

    async def init_wolf(self):
    ...
    async def on_command(self, ctx: Context):
        ...
        await User.get_or_create(user_id=ctx.author.id)

    async def on_ready(self):
        print(f"[Wolf] Logged in as {self.user.name}({self.user.id})")
    ...
    async def is_premium_user(user_id: int) -> bool:
        ...
        return await User.filter(pk=user_id, is_premium=True).exists()

    @staticmethod
    @cached(ttl=60)
    async def is_premium_user_or_guild(user_id: int, guild_id: int) -> bool:
        from models import User
        user_premium = await User.filter(pk=user_id, is_premium=True).exists()
        if user_premium:
            return True
        return await Guild.filter(pk=guild_id, is_premium=True).exists()
