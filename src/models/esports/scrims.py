...
class Scrim(BaseDbModel):
    ...
    name = fields.TextField(default="Wolf-Scrims")
    ...
    slots_update_channel_id = fields.BigIntField(null=True)
    slots_reminder_thresholds = ArrayField(fields.IntField(), default=list)
    slots_reminder_role_id = fields.BigIntField(null=True)
    ...
    def logschan(self):
        if self.guild is not None:
            return discord.utils.get(self.guild.text_channels, name="wolf-scrims-log")
    ...
    @property
    def slots_update_channel(self):
        return self.bot.get_channel(self.slots_update_channel_id)

    @property
    def slots_reminder_role(self):
        if self.guild is not None:
            return self.guild.get_role(self.slots_reminder_role_id)
    ...
    def check_emoji(self):
        return self.emojis.get("tick", "<a:ggdhominoj:1426522110003773450>")

    @property
    def cross_emoji(self):
        return self.emojis.get("cross", "<:aadat_toxic:1426620476817408071>")
    ...
    async def send_slotlist(self, channel: discord.TextChannel = None) -> discord.Message:
        ...
        embed, _ = await self.create_slotlist()
        _v.message = await channel.send(embed=embed, view=_v)

        if channel == self.slotlist_channel:
    ...
    async def create_slotlist_template_img(self):
        """
        Create slotlist using custom template image
        """
        slots = await self.teams_registered
        ...
        async with aiohttp.ClientSession() as session:
            for slot in slots:
                try:
                    user = self.bot.get_user(slot.user_id)
                    if user:
                    ...
                    async with session.get(avatar_url) as resp:
                        if resp.status == 200:
                            ...
                            avatar_images[slot.num] = avatar_data
                except Exception as e:
                    print(f"Error downloading avatar for slot {slot.num}: {e}")
        
        def wrapper():
            try:
                template_path = Path.cwd() / "data" / "images" / "slotlist_template.png"
                ...
                # Load font matching the template design (robo-italic for team names)
                font_path = Path.cwd() / "data" / "font" / "robo-italic.ttf"
                print(f"Font path: {font_path}")
                print(f"Font exists: {font_path.exists()}")
                team_font = ImageFont.truetype(str(font_path), 28)
            except Exception as e:
                print(f"Error in wrapper initialization: {e}")
                import traceback
                traceback.print_exc()
                raise
            
            # Position configuration for left side (slots 1-12)
            left_logo_x = 205  # X position for logos on left side
            left_team_x = 245  # X position for team names on left side
            left_start_y = 358  # Y position for first slot
            
            # Position configuration for right side (slots 13+)
            right_logo_x = 515  # X position for logos on right side
            right_team_x = 555  # X position for team names on right side
            right_start_y = 358  # Y position for first slot
            
            line_height = 58  # Height between each slot line (architect confirmed)
            logo_size = 32  # Size of the logo (32x32 pixels)
            
            # Draw logos and team names next to the numbers already on template
            for idx, slot in enumerate(slots):
                team_text = slot.team_name[:22]  # Limit length to fit
                
                if slot.num <= 12:
                    # Left side positioning
                    text_y = left_start_y + ((slot.num - 1) * line_height)
                    logo_y = text_y - 2  # Adjust logo slightly up for better centering
                    
                    # Draw logo if available
                    if slot.num in avatar_images:
                        try:
                            avatar_img = Image.open(io.BytesIO(avatar_images[slot.num]))
                            avatar_img = avatar_img.resize((logo_size, logo_size), Image.Resampling.LANCZOS)
                            ...
                            # Paste with mask
                            template.paste(avatar_img, (left_logo_x, logo_y), mask)
                        except:
                            pass
                    
                    # Draw team name
                    draw.text((left_team_x, text_y), team_text, font=team_font, fill="#2b4a5e")
                else:
                    # Right side positioning
                    text_y = right_start_y + ((slot.num - 13) * line_height)
                    logo_y = text_y - 2  # Adjust logo slightly up for better centering
                    ...
                    if slot.num in avatar_images:
                        try:
                            ...
                            mask_draw.ellipse((0, 0, logo_size, logo_size), fill=255)
                            # Paste with mask
                            template.paste(avatar_img, (right_logo_x, logo_y), mask)
                        except:
                            pass
                    
                    # Draw team name
                    draw.text((right_team_x, text_y), team_text, font=team_font, fill="#2b4a5e")
            
            # Save to bytes
            img_bytes = io.BytesIO()
            template.save(img_bytes, "PNG")
            img_bytes.seek(0)
            
            return discord.File(img_bytes, "slotlist.png")
